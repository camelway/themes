...
<ol itemscope itemtype="https://schema.org/BreadcrumbList">
            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                <a href="$homeurl" itemprop="item"><span itemprop="name">Bosh sahifa</span></a>
...
function webp2jpg(){
    $image = ABSPATH.str_replace('https://uz.camelway.uz/', '', urldecode($_REQUEST['f']));
...
function og_image(){
    if(is_home()){
        return array('http://uz.camelway.uz/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
    }
    ...
    if(substr($url, 0 , 23) == 'https://uz.camelway.uz/'){
        $url = str_replace('https://uz.camelway.uz/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://uz.camelway.uz/'.$url, $w, $h);
}
...
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
       ...
       if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
           ...
           elseif(strpos($imgsrc, 'https://uz.camelway.uz/') === 0){
                $imgsrc = ABSPATH.substr($imgsrc, 23);
           }else
