...
function webp2jpg(){
    $url = str_replace('https://www.camelway.pe/', '', $_REQUEST['f']);
    $url = str_replace('http://www.camelway.pe/', '', $url);
...
function og_image(){
    if(is_home()){
        return array('http://www.camelway.pe/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
    }
    ...
    if(substr($url, 0 , 24) == 'https://www.camelway.pe/'){
        $url = str_replace('https://www.camelway.pe/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://www.camelway.pe/'.$url, $w, $h);
}
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
       ...
       if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
           ...
           elseif(strpos($imgsrc, 'https://www.camelway.pe/') === 0){
