...
<ol itemscope itemtype="https://schema.org/BreadcrumbList">
            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                <a href="$homeurl" itemprop="item"><span itemprop="name">дома</span></a>
...
function show_1_mobile($mobile){
    $mobile_arr = explode(",", $mobile);
    shuffle($mobile_arr);
    $lmobile = array_splice($mobile_arr, 0, 1);
    return implode(', ', $lmobile);
}

function webp2jpg(){
    $image = ABSPATH.str_replace('https://www.camelway.uz/', '', urldecode($_REQUEST['f']));
...
function og_image(){
    if(is_home()){
        return array('http://www.camelway.uz/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
    }
    elseif(is_single() || is_topic() || is_page() || is_category() || is_search()){
        if(have_posts()){
    ...
    if(substr($url, 0 , 24) == 'https://www.camelway.uz/'){
        $url = str_replace('https://www.camelway.uz/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://www.camelway.uz/'.$url, $w, $h);
}
...
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
       ...
       if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
           ...
           elseif(strpos($imgsrc, 'https://www.camelway.uz/') === 0){
