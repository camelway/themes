...
function webp2jpg(){
    $image = ABSPATH.str_replace('https://www.camelway.co.za/', '', urldecode($_REQUEST['f']));
...
function og_image(){
    if(is_home()){
        return array('http://www.camelway.co.za/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
    }
    ...
    if(substr($url, 0 , 27) == 'https://www.camelway.co.za/'){
        $url = str_replace('https://www.camelway.co.za/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://www.camelway.co.za/'.$url, $w, $h);
}
...
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
        ...
        if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
            ...
            elseif(strpos($imgsrc, 'https://www.camelway.co.za/') === 0){
                $imgsrc = ABSPATH.substr($imgsrc, 27);
            }else
