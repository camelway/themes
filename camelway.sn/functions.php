...
function webp2jpg(){
    $image = ABSPATH.str_replace('https://www.camelway.sn/', '', $_REQUEST['f']);
...
function og_image(){
    ...
    if(substr($url, 0 , 27) == 'https://www.camelway.sn/'){
        $url = str_replace('https://www.camelway.sn/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://www.camelway.sn/'.$url, $w, $h);
}
...
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
       ...
       if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
           ...
           elseif(strpos($imgsrc, 'https://www.camelway.sn/') === 0){
