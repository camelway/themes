...
function add_title($title){
    if(!is_home())
        return $title.' - '.'Camelway Sri Lanka';
    return $title;
}
...
function webp2jpg(){
    $url = str_replace('https://www.camelway.lk/', '', $_REQUEST['f']);
    $url = str_replace('http://www.camelway.lk/', '', $url);
    $image = ABSPATH.urldecode($url);
...
function og_image(){
    if(is_home()){
        return array('http://www.camelway.lk/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
    }
    ...
    if(substr($url, 0 , 24) == 'https://www.camelway.lk/'){
        $url = str_replace('https://www.camelway.lk/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://www.camelway.lk/'.$url, $w, $h);
}
...
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
       ...
       if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
           ...
           elseif(strpos($imgsrc, 'https://www.camelway.lk/') === 0){
