...
<?php } if(is_home()){?>
    <script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"Organization","@id":"<?php echo home_url(1)?>#organization","name":"Camelway Machinery","url":"<?php echo home_url(1)?>","sameAs":["https://fr.camelway.com/","https://www.facebook.com/camelway/"],"logo":{"@type":"ImageObject","@id":"<?php echo home_url(1)?>#logo","inLanguage":"fr-FR","url":"https://data.camelway.net/static/images/camelway-500x500.png","width":500,"height":500,"caption":"Camewlay Africa Logo"},"image":{"@id":"<?php echo home_url(1)?>#logo"}},{"@type":"WebSite","@id":"<?php echo home_url(1)?>#website","url":"<?php echo home_url(1)?>","name":"Camelway Machinery","inLanguage":"fr-FR","description":"sell construction machine and provide construction machine information in africa","publisher":{"@id":"<?php echo home_url(1)?>#organization"},"potentialAction":[{"@type":"SearchAction","target":"<?php echo home_url(1)?>?s={search_term_string}","query-input":"required name=search_term_string"}]},{"@type":"WebPage","@id":"<?php echo home_url(1)?>#webpage","url":"<?php echo home_url(1)?>","name":"Camelway Machinery | Concrete Batching Plant | Asphalt Mixing Plant| Stone Crushing Plant | Block Making Plant | Concrete Mixer | Concrete Pump | Rebar Equipment","isPartOf":{"@id":"<?php echo home_url(1)?>#website"},"inLanguage":"fr-FR","about":{"@id":"<?php echo home_url(1)?>#organization"},"datePublished":"<?php the_time('Y-m-d h:i:sP', 1);?>","dateModified":"<?php the_modified_time('Y-m-d h:i:sP')?>","description":"provide construction machine sale information in africa include batching plant, asphalt plant, crushing plant, concrete mixer, cocnrete pump, rebar equipment, etc","potentialAction":[{"@type":"ReadAction","target":["<?php echo home_url(1)?>"]}]}]}</script>
<?php }elseif(is_category()){?>
    <script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"Organization","@id":"<?php echo home_url(1)?>#organization","name":"Camelway Machinery","url":"<?php echo home_url(1)?>","sameAs":["https://fr.camelway.com/","https://www.facebook.com/camelway/"],"logo":{"@type":"ImageObject","@id":"<?php echo home_url(1)?>#logo","inLanguage":"fr-FR","url":"https://data.camelway.net/static/images/camelway-500x500.png","width":500,"height":500,"caption":"Camewlay Africa Logo"},"image":{"@id":"<?php echo home_url(1)?>#logo"}},{"@type":"WebSite","@id":"<?php echo home_url(1)?>#website","url":"<?php echo home_url(1)?>","name":"Camelway Machinery","inLanguage":"fr-FR","description":"sell construction machine and provide construction machine information in africa","publisher":{"@id":"<?php echo home_url(1)?>#organization"},"potentialAction":[{"@type":"SearchAction","target":"<?php echo home_url(1)?>?s={search_term_string}","query-input":"required name=search_term_string"}]},{"@type":"CollectionPage","@id":"<?php the_category_link()?>#webpage","url":"<?php  the_category_link()?>","name":"<?php the_category_name()?>","isPartOf":{"@id":"<?php echo home_url(1)?>#website"},"inLanguage":"fr-FR","description":"<?php dm_description(180)?>..."}]}</script>
<?php }elseif(is_single() && in_category(array(1,2,3,4,5,6,7))) { ?>
    <script type="application/ld+json">{"@context":"https://schema.org/","@type":"Product","name":"<?php the_subtitle()?>","image":[<?php $images = get_post_images('gallery'); $tmp = '"'.get_the_thumbnail().'",';foreach($images as $image){ $tmp .= sprintf('"%s",', $image->url);} echo trim($tmp, ',')?>],"description":"<?php echo dm_trim_words(get_the_excerpt(), 200);?>","sku":"p-<?php the_ID()?>","mpn":"<?php echo the_category_slug().'-'.get_the_ID();?>","brand":{"@type":"Brand","name":"Camelway"},"aggregateRating":{"@type":"AggregateRating","ratingValue":"<?php the_ratingValue();?>","reviewCount":"<?php the_reviewCount();?>","bestRating":"5","worstRating":"1"},"offers":{"@type":"AggregateOffer","url":"<?php the_permalink()?>#feedback","priceCurrency":"USD","availability":"https://schema.org/PreOrder","itemCondition":"http://schema.org/NewCondition","lowPrice":"<?php the_post_data('lowprice')?>","highPrice":"<?php the_post_data('highprice')?>","seller":{"@type":"Organization","name":"Camelway Machinery"}}}</script>
<?php }elseif(is_single() && in_category(array(8,9))){?>
    <script type="application/ld+json">{"@context": "http://schema.org","@type": "BlogPosting","headline": "<?php the_title()?>","alternativeHeadline":"<?php the_subtitle()?>","datePublished": "<?php the_time('Y-m-d');?>","dateModified": "<?php the_modified_time('Y-m-d');?>","image":[<?php $images = get_post_images('gallery'); $tmp = '"'.get_the_thumbnail().'",';foreach($images as $image){ $tmp .= sprintf('"%s",', $image->url);} echo trim($tmp, ',')?>],"keywords":[<?php echo trim(get_the_tag_list('"','", "','"'),',')?>],"mainEntityOfPage":"<?php the_permalink();?>","description":"<?php echo dm_trim_words(get_the_excerpt(), 160)?>","author": "<?php the_author()?>","publisher":{"@type": "Organization","name": "Camelway Machinery","url": "https://www.camelway.fr","logo": {"@type": "ImageObject","url": "https://data.camelway.net/static/images/camelway-114x114.png","width":"114","height":"114"}}}</script>
<?php } ?>
    ...
    .feedback .result {float:left;width:calc(90% - 96px)}
    ...
    .category-thumb {margin-top:10px;background:#fff}
    .category-thumb h2 {font-size:14px}
    .category-thumb ul {display:flex;flex-wrap:wrap;justify-content:space-between;list-style:none}
    .category-thumb ul li {width:100%;padding:5px;box-sizing:border-box;border-bottom:1px solid #eee}
    .category-thumb .images {display:flex;flex-wrap:no-wrap;justify-content:space-between;}
    .category-thumb .images a {display:block;width:32.5%}
    .category-thumb .pubmeta {width:100%;margin-top:10px;font-size:12px;color:#919191}
    ...
    .single-header h1.aligncenter {width:100%;margin-bottom:5px;text-align:center}
...
<header class="wrap">
    ...
    <div class="menu">
        <ul>
            <li><a href="<?php the_mobile_permalink(41)?>">À propos de nous</a></li>
            <li><a href="<?php the_mobile_permalink(42)?>">Nous contacter</a></li>
