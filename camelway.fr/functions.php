...
<ol itemscope itemtype="https://schema.org/BreadcrumbList">
            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                <a href="$homeurl" itemprop="item"><span itemprop="name">Accueil</span></a>
...
function webp2jpg(){
    ...
    $image = ABSPATH.str_replace('https://www.camelway.fr/', '', $f);
    ...
    header("Link: <".esc_url($f).">; rel='canonical'");
...
function og_image(){
    if(is_home()){
        return array('http://www.camelway.fr/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
    }
    elseif(is_single() || is_search()){
        ...
        $url = get_category_thumbnail(); 
        if(empty($url))
            return array('http://www.camelway.fr/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
        return array($url, 600, 308);
    }elseif(is_author()){
        $url = get_the_author_meta('avatar');
    }
    if(substr($url, 0 , 24) == 'https://www.camelway.fr/'){
        $url = str_replace('https://www.camelway.fr/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://www.camelway.fr/'.$url, $w, $h);
}
...
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
        ...
        if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
            ...
            elseif(strpos($imgsrc, 'https://www.camelway.fr/') === 0){
...
function hide_no_thumbnail($query){
    if($query->is_main_query() && $query->is_category && !$query->is_mobile){
