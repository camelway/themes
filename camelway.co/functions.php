...
function webp2jpg(){
    $image = ABSPATH.str_replace('https://www.camelway.co/', '', urldecode($_REQUEST['f']));
...
function og_image(){
    if(is_home()){
        return array('https://www.camelway.co/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
    }
    ...
    if(substr($url, 0 , 24) == 'https://www.camelway.co/'){
        $url = str_replace('https://www.camelway.co/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://www.camelway.co/'.$url, $w, $h);
}
...
function hide_no_thumbnail($query){
    if($query->is_category && $query->is_main_query() && !$query->is_mobile){
...
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
       ...
       if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
           ...
           elseif(strpos($imgsrc, 'https://www.camelway.co/') === 0){
