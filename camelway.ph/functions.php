...
function webp2jpg(){
    $url = str_replace('https://www.camelway.ph/', '', $_REQUEST['f']);
    $url = str_replace('http://www.camelway.ph/', '', $url);
...
function og_image(){
    if(is_home()){
        return array('http://www.camelway.ph/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
    }
    ...
    if(substr($url, 0 , 27) == 'https://www.camelway.ph/'){
        $url = str_replace('https://www.camelway.ph/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://www.camelway.ph/'.$url, $w, $h);
}
...
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
       ...
       if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
           ...
           elseif(strpos($imgsrc, 'https://www.camelway.ph/') === 0){
