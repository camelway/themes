...
<?php } elseif(count($images) <= 2){ ?>
            ...
            <div class="item">
                <img src="<?php echo dminfo('ajax_url').'?action=cropimage&f='.urlencode($images[0]['url']).'&width=200'?>" alt="<?php the_title()?>" width="200" height="150" class="thumbnail">
                <h2><a href="<?php the_permalink();?>"><?php the_subtitle()?></a></h2>
                <p><?php echo dm_trim_words(get_the_excerpt(), 255);?></p>
                <span class="pubdate"><?php the_time('M j, Y H:i')?></span>
            </div>
...
foreach($images as $key=>$image){
    if($key==3)
        break;
?>
                    <a href="<?php the_permalink()?>"><img src="<?php echo dminfo('ajax_url').'?action=cropimage&f='.urlencode($image['url']).'&width=315'?>" alt="<?php the_title()?>" width="315" height="235"></a>
<?php } ?>
                </div>
                <span class="pubdate"><?php the_time('M j, Y H:i')?></span>
            </div>
