...
<ol itemscope itemtype="https://schema.org/BreadcrumbList">
            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                <a href="$homeurl" itemprop="item"><span itemprop="name">Главная страница</span></a>
...
function webp2jpg(){
    $image = ABSPATH.str_replace('https://www.camelway.ru/', '', urldecode($_REQUEST['f']));
...
function og_image(){
    if(is_home()){
        return array('http://www.camelway.ru/dm-content/themes/hi-domai/images/camelway-china.jpg', 600, 388);
    }
    ...
    if(substr($url, 0 , 24) == 'https://www.camelway.ru/'){
        $url = str_replace('https://www.camelway.ru/', '', $url);
    }
    $path = ABSPATH.$url;
    ...
    return array('https://www.camelway.ru/'.$url, $w, $h);
}
...
function hide_no_thumbnail($query){
    if($query->is_category && !$query->is_mobile() && $query->is_main_query()){
...
function amp_filter($content){
    ...
    $mipcontent = preg_replace_callback('/<amp-img\s+([^>]*)><\/amp-img>/iS', function($m){
       ...
       if((empty($attrs['width']) || empty($attrs['height'])) && function_exists('getimagesize')){
           ...
           elseif(strpos($imgsrc, 'https://www.camelway.ru/') === 0){
